...
bot.action('check_subscription', async (ctx) => {
  try {
    ...
    if (isSubscribed) {
      ...
      const successMessage = await ctx.reply('✅ Вы подписаны на все необходимые каналы!', getMainKeyboard());
      ...
      setTimeout(async () => {
        try {
          await ctx.telegram.deleteMessage(successMessage.chat.id, successMessage.message_id);
        } catch (e) {
